...
docker run -d --name target --rm --network mynetwork -p 80:80 -v $(pwd):/shared_volume ubuntu:20.04 /bin/bash -c 'apt-get update; apt-get install -y openssh-server;service ssh start; while true; do sleep 60; echo keepalive; done'  
